...
if [ "$SKIP_MIRROR" == "true" ]; then
    ...
    if [ -n "$LOCAL_REGISTRY" ]; then
        echo -e "${GREEN}✓ Local registry detected: ${LOCAL_REGISTRY}${NC}"
        ...
        if [ -z "$IPV4_SERVICE" ]; then
            ...
            echo -e "${YELLOW}⚠ Nodes are rebooting to apply new image policy${NC}"
            ...
            echo -e "${GREEN}✓ Cluster has IPv4 connectivity - direct image pull available${NC}"
            echo "  Image mirroring not required"
            echo ""
        fi
    else
        echo -e "${YELLOW}⚠ No local registry detected${NC}"
