...
#   --che-server-image <image>   Che server image (default: quay.io/eclipse/che-server:next)
#   --che-operator-cr-yaml <file>       Path to custom CheCluster CR yaml (full definition)
#   --che-operator-cr-patch-yaml <file> Path to CheCluster CR patch yaml (overrides only specified fields)
...
#
# Note: Timeouts for chectl are automatically increased 2x for IPv6-only clusters:
#   - Pod wait timeout: 240s (default 120s)
#   - Pod ready timeout: 240s (default 120s)
#   - Pod error recheck: 120s (default 60s)
#   - Image download: 2400s/40min (default 1200s/20min)
#
...
# chectl timeout values (2x defaults for IPv6-only clusters)
# Defaults from chectl: wait=120s, ready=120s, error-recheck=60s, download=1200s
# Increased values for slow IPv6 connections
CHECTL_K8S_POD_WAIT_TIMEOUT=240000          # 240s (was 120s)
CHECTL_K8S_POD_READY_TIMEOUT=240000         # 240s (was 120s)
CHECTL_K8S_POD_ERROR_RECHECK_TIMEOUT=120000 # 120s (was 60s)
CHECTL_K8S_POD_DOWNLOAD_IMAGE_TIMEOUT=2400000 # 2400s/40min (was 1200s/20min)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --che-operator-cr-yaml)
            CHE_OPERATOR_CR_YAML="$2"
            shift 2
            ;;
        --che-operator-cr-patch-yaml)
            CHE_OPERATOR_CR_PATCH_YAML="$2"
            shift 2
            ;;
        --crw)
            DEPLOY_CRW=true
...
if [ "${DEPLOY_CRW}" = "true" ]; then
    ...
    CHE_SERVER_IMAGE="${CHE_SERVER_IMAGE:-registry.redhat.io/codeready-workspaces/server-rhel8:latest}"
    ...
    CHE_SERVER_IMAGE="${CHE_SERVER_IMAGE:-quay.io/eclipse/che-server:next}"
...
# chectl has hardcoded 120s timeout for OLM subscription readiness (src/api/kube-client.ts:1395)
# We kill the process before timeout on attempts 1-2 and retry up to 3 times
# The increased pod timeouts (240s+) help after subscription is ready, giving more time for pods
CHECTL_MAX_RETRIES=3
CHECTL_TIMEOUT_SECONDS=110  # Kill before chectl's hardcoded 120s OLM subscription timeout (attempts 1-2)
CHECTL_ATTEMPT=1

echo -e "${BLUE}Using increased chectl timeouts for IPv6-only cluster:${NC}"
echo "  Pod wait timeout:          ${CHECTL_K8S_POD_WAIT_TIMEOUT}ms ($(( CHECTL_K8S_POD_WAIT_TIMEOUT / 1000 ))s)"
echo "  Pod ready timeout:         ${CHECTL_K8S_POD_READY_TIMEOUT}ms ($(( CHECTL_K8S_POD_READY_TIMEOUT / 1000 ))s)"
echo "  Pod error recheck timeout: ${CHECTL_K8S_POD_ERROR_RECHECK_TIMEOUT}ms ($(( CHECTL_K8S_POD_ERROR_RECHECK_TIMEOUT / 1000 ))s)"
echo "  Image download timeout:    ${CHECTL_K8S_POD_DOWNLOAD_IMAGE_TIMEOUT}ms ($(( CHECTL_K8S_POD_DOWNLOAD_IMAGE_TIMEOUT / 1000 ))s)"
echo ""
...
deploy_with_chectl() {
    ...
    # Prepare chectl arguments array for cleaner command construction
    local CHECTL_ARGS=(
        --platform openshift
        --installer operator
        --batch
        ...
        --k8spodwaittimeout "${CHECTL_K8S_POD_WAIT_TIMEOUT}"
        --k8spodreadytimeout "${CHECTL_K8S_POD_READY_TIMEOUT}"
        --k8spoderrorrechecktimeout "${CHECTL_K8S_POD_ERROR_RECHECK_TIMEOUT}"
        --k8spoddownloadimagetimeout "${CHECTL_K8S_POD_DOWNLOAD_IMAGE_TIMEOUT}"
    )

    # Add optional Che server image
    if [ -n "${CHE_SERVER_IMAGE}" ]; then
        CHECTL_ARGS+=(--cheimage "${CHE_SERVER_IMAGE}")
    fi

    # Add optional custom CheCluster CR yaml
    if [ -n "${CHE_OPERATOR_CR_YAML}" ]; then
        CHECTL_ARGS+=(--che-operator-cr-yaml "${CHE_OPERATOR_CR_YAML}")
    fi

    # Add optional CheCluster CR patch yaml
    if [ -n "${CHE_OPERATOR_CR_PATCH_YAML}" ]; then
        CHECTL_ARGS+=(--che-operator-cr-patch-yaml "${CHE_OPERATOR_CR_PATCH_YAML}")
    fi

    set +e
    KUBECONFIG="${CHECTL_KUBECONFIG}" HTTPS_PROXY="${PROXY_URL:-}" HTTP_PROXY="${PROXY_URL:-}" NO_PROXY="${CHECTL_NO_PROXY}" \
        chectl server:deploy "${CHECTL_ARGS[@]}" &
